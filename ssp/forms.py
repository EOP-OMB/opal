...
class system_security_plansForm(forms.Form):
    ...
    published = forms.DateTimeField(help_text='The date and time the document was published. The date-time value must be formatted according to RFC 3339 with full time and time zone included.', label='Publication Timestamp')
    last_modified = forms.DateTimeField(help_text='The date and time the document was last modified. The date-time value must be formatted according to RFC 3339 with full time and time zone included.', label = 'Last Modified Timestamp')
    ...
    import_profile = forms.ModelChoiceField(queryset=ctrl_profiles.objects.all(),label='Import profile')
    ...
    class Meta:
        ...
        widgets = {'published': SelectDateWidget,
                   'last_modified': SelectDateWidget}
        initial = {'published': datetime.now(),
