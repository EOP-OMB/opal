...
def getProperties(obj):
    ...
    #     prop_list = []
    #     for p in obj:
    #         prop = {'value': p.value, 'name': p.name, 'uuid': p.property_id, 'ns': p.ns, 'class': p.prop_class}
...
def getLinks(obj):
    ...
    #     link_list = []
    #     for l in obj:
    #         link = {'text': l.text, 'href': l.href, 'rel': l.rel, 'media-type': l.mediaType}
...
def getRoles(obj):
    if not obj.exists():
        ...
        roles_list = []
        for r in obj:
            role = {'id': r.id, 'title': r.title, 'short-name': r.shortName, 'desc': r.desc,
                    'properties': getProperties(r.properties.all()), 'annotations': getAnnotations(r.annotations.all()),
                    'links': getLinks(r.links.all()), 'remarks': r.remarks}
...
def getAnnotations(obj):
    if not obj.exists():
        ...
        annotations_list = []
        for a in obj:
            annotation = {'name': a.name, 'uuid': a.annotationID, 'ns': a.ns, 'value': a.value, 'remarks': a.remarks}
